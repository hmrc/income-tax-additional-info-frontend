...
object CheckAnswersQualifyingLoanSummary  {

  def summary(taxYear: Int)(implicit request: JourneyDataRequest[_], messages: Messages): SummaryList =
    SummaryList(Seq(
      request.userAnswers.get(QualifyingLoanReliefPage).map { answer =>
        SummaryListRow(
          key = Key(
            content = Text(messages("business-reliefs.qualifying-loan.amount.question")),
          ...
          actions =
            Some(Actions(items = Seq(
              ActionItem(
                ...
                visuallyHiddenText = Some(messages("business-reliefs.qualifying-loan.amount.question"))
              )
            )))
        )
      }
