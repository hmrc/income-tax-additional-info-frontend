...
object DeleteGainsParser extends Parser with Logging {
  type DeleteGainsResponse = Either[ApiError, Boolean]

  override val parserName: String = "DeleteGainsHttpParser"
  override val service: String = "income-tax-additional-information"

  implicit object DeleteGainsHttpReads extends HttpReads[DeleteGainsResponse] {
    override def read(method: String, url: String, response: HttpResponse): DeleteGainsResponse = response.status match {
