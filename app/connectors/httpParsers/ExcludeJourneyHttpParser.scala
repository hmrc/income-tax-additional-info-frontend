...
object ExcludeJourneyHttpParser extends Parser {

  type ExcludeJourneyResponse = Either[ApiError, Int]
  ...
  implicit object ExcludeJourneyResponseReads extends HttpReads[ExcludeJourneyResponse] {
    override def read(method: String, url: String, response: HttpResponse): ExcludeJourneyResponse = {
      response.status match  {
        ...
        case BAD_REQUEST => handleError(response, BAD_REQUEST)
        case INTERNAL_SERVER_ERROR => handleError(response, INTERNAL_SERVER_ERROR)
        case _ => handleError(response, INTERNAL_SERVER_ERROR)
      }
    }
  }
}
