...
class EncryptionServiceISpec extends IntegrationTest {
  ...
  "encryptGainsUserData" should {
    ...
    val data: GainsUserDataModel = GainsUserDataModel("sessionId", "mtditid", "AA123456A", 1999, Some(AllGainsSessionModel(Seq(completePolicyCyaModel), gateway = Some(true))), DateTime.now(DateTimeZone.UTC))
