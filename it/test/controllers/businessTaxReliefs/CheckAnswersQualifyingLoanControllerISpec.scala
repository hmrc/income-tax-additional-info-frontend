...
class CheckAnswersQualifyingLoanControllerISpec extends IntegrationTest with UserAnswersStub with BeforeAndAfterEach {
  ...
  private def url(taxYear: Int): String =
    s"/update-and-submit-income-tax-return/additional-information/$taxYear/business-reliefs/qualifying-loan-interest/check-answers"

  lazy val userAnswers: UserAnswersModel =
    emptyUserAnswers(taxYear, BusinessTaxReliefs).set(QualifyingLoanReliefPage, BigDecimal(22.45))

  ".show" when {

    "user is an Agent" should {

      "render the Check Answers Qualifying Loan Interest page" in {
        ...
        contentAsString(result) must include(BusinessReliefsQualifyingLoanMessages.EnglishAgent.cya)
    ...
    "user is an Individual" should {
      ...
      "render the Check Answers Qualifying Loan Interest page" in {
        ...
        status(result) mustEqual OK
        contentAsString(result) must include(BusinessReliefsQualifyingLoanMessages.English.cya)
