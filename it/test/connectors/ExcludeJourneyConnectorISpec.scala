  ...
  ".excludeJourney" should {
    ...
    "return a BadRequest" when {
      ...
      "the backend returns a bad request" in {
        ...
        await(result) shouldBe Left(ApiError(BAD_REQUEST, SingleErrorBody("IT_WRONG", "It bad yoh")))
      }
    }
    ...
    "return an InternalServerError" when {
      ...
      "the backend returns an internal server error" in {
        ...
        await(result) shouldBe Left(ApiError(INTERNAL_SERVER_ERROR, SingleErrorBody("WE_GON_GOOFED", "We messed up, my bad.")))
      }
      ...
      "the backend returns an unexpected answer" in {
        ...
        await(result) shouldBe Left(ApiError(INTERNAL_SERVER_ERROR, SingleErrorBody("TEAPOT", "Imma teapot")))
      }
    }
  }
