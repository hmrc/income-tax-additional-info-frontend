  ...
  "GainsPriorDataModel" should {
    ...
    "correctly map to PolicyCyaModel" when {
      "each prior model has data" in {
        ...
        val lifeInsuranceModel: PolicyCyaModel = PolicyCyaModel(result.head.policyId, Some("Life Insurance"), None, Some(123.11), None, Some(false), None, None, None, None, Some(false), None)
        result shouldBe
          List(
            lifeInsuranceModel,
            lifeInsuranceModel.copy(policyId = result(1).policyId, policyType = Some("Capital Redemption"), treatedAsTaxPaid = Some(false)),
            lifeInsuranceModel.copy(policyId = result(2).policyId, policyType = Some("Life Annuity")),
            lifeInsuranceModel.copy(policyId = result(3).policyId, policyType = Some("Voided ISA"), treatedAsTaxPaid = Some(false)),
            lifeInsuranceModel.copy(policyId = result(4).policyId, policyType = Some("Foreign Policy"), policyEvent = Some(""), yearsPolicyHeldPrevious = Some(0),
