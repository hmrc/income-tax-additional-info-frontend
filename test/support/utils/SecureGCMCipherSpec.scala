...
class SecureGCMCipherSpec extends UnitTest {
  ...
  private implicit lazy val appConfig: AppConfig = new AppConfigStub().config()

  private lazy val noEncryptionAppConfig = new AppConfigStub().noEncryptionConfig()
  ...
  "encrypt" should {
    "return plain text when turned off" in {
      val encrypterWithNoCrypt = new SecureGCMCipher()(noEncryptionAppConfig)
