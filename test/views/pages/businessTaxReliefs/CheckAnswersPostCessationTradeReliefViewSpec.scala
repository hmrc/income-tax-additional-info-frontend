...
class CheckAnswersPostCessationTradeReliefViewSpec extends ViewUnitTest {

  private val page: CheckAnswersPostCessationTradeReliefView = inject[CheckAnswersPostCessationTradeReliefView]
  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      ...
      "render post-cessation trade relief check your answerssbt " which {
        ...
        textOnPageCheck(expectedMessages.checkAnswerQuestion, Selectors.summaryKey(row = 1))
